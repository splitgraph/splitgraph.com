export const meta = {
  id: "sql-endpoint",
  title: "Splitgraph SQL endpoint",
};

**Note that this feature is currently in a closed beta. E-mail us at support@splitgraph.com with your username if you would like to gain access.**

Splitgraph SQL endpoint, or Splitgraph Data Delivery Network (DDN) is a feature of Splitgraph Cloud. It's a PostgreSQL-compatible SQL endpoint hosted at `postgres://data.splitgraph.com:5432/ddn` that lets you query any dataset hosted on or indexed by Splitgraph.

In an SQL query, you can reference Splitgraph images as PostgreSQL schemas and even run JOINs across multiple images or data sources. For example:
...
You need to have logged in to Splitgraph cloud. See the [Splitgraph Cloud introduction](../introduction) for more information.
...
You don't need to install anything to use the endpoint. If you go to your Splitgraph [account settings](https://www.splitgraph.com/settings), you can generate a pair of credentials that you can immediately plug into any SQL client.
