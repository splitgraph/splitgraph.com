...
One such tool is [PostgREST](https://postgrest.org/en/latest/) that generates a REST API for a PostgreSQL schema. Splitgraph runs PostgREST in [Splitgraph Cloud](../splitgraph-cloud/publish-rest-api), allowing any Splitgraph dataset to be accessed via HTTP.
