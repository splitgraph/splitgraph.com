...
Splitgraph tries to not redo work that can be done more efficiently by the database, which is why
after an image is checked out into native PostgreSQL tables, Splitgraph stops getting involved.

However, this becomes an issue with large datasets where the overhead of checking out an image is
too large. This is why Splitgraph supports querying images directly without materializing
them into actual PostgreSQL tables.
...
This is similar to Docker, where an application running inside of a container thinks that it's interacting
...
Splitgraph simply downloads required objects and materializes required table regions
and tells PostgreSQL where it can find the query result. This lets layered querying compete with and
sometimes beat PostgreSQL on query performance.

Layered querying is currently read-only.

The Splitfile executor uses layered querying to satisfy `IMPORT` and `SQL` commands that read from
other Splitgraph images.

## Layered checkouts

To use layered querying, check the dataset out with the `--layered` flag. For example:
