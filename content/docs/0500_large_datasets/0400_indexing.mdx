...
used by [layered querying](large_datasets/layered_querying) to determine which objects need to
...
doesn't or might have a certain value. [The example](#reindexing-example) shows adding a bloom filter index on the
county name to a dataset of precinct-level results of the 2016 US Presidential Election, allowing Splitgraph
to filter on county names with just 1KB of filter data and a 1% false positive rate.
...
Reindexing is currently only supported for objects that haven't been delta compressed (no overlapping objects). Images with overlapping objects should be [rechunked](large_datasets/chunking) first.

To reindex objects that a table consists of, use [`sgr reindex`](../sgr/image_management_creation/reindex). For example:
...
This will add the new index to the object metadata, keeping existing ones.

Note that if you wish to push new object metadata after reindexing, you'll need to add
`--overwrite-object-meta` to your [`sgr push`](../sgr/sharing_images/push) command. Pushed objects are not overwritten by default.

## Reindexing example

<cast src="@splitgraph/content/casts/versioned/latest/bloom-filter.cast" />
[Link to example](https://github.com/splitgraph/splitgraph/tree/master/examples/bloom-filter)
