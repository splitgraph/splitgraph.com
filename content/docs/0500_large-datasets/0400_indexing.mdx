...
Reindexing is currently only supported for objects that haven't been delta compressed (no overlapping objects). Images with overlapping objects should be [rechunked](large-datasets/chunking) first.
