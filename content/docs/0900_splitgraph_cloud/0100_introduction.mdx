...
Every dataset pushed out to the Splitgraph registry gets multiple perks. One of them is an automatic read-only OpenAPI-compatible endpoint that can be used to query it, provided by [PostgREST](postgrest.org/).
...
To log into the registry again or get a new set of API keys, use [`sgr cloud login`](../sgr/splitgraph_cloud/cloud_login).