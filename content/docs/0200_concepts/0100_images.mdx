...
There are multiple ways to create Splitgraph images. The easiest one is by checking an image
out into an actual PostgreSQL schema. This enables change tracking and allows any application that
can interact with a PostgreSQL database to work with Splitgraph. At any point in time, the schema
can be committed to create a new image.
