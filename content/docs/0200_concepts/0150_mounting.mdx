...
For [external repositories](../splitgraph-cloud/external-repositories) that are indexed but not hosted by Splitgraph, mounting is the only way to query them. You can do so through the [Splitgraph DDN](../splitgraph-cloud/data-delivery-network) or by [mounting them manually](../ingesting-data/foreign-data-wrappers/socrata) on a local engine.
...
Mounting is powered by PostgreSQL [foreign data wrappers](../ingesting-data/foreign-data-wrappers/introduction) and we added some scaffolding to them to make it easy to create foreign tables from the `sgr` client. Splitgraph also ships with a few open-source foreign data wrappers, letting you query other PostgreSQL/MySQL databases, MongoDB collections or even [government open data](../ingesting-data/foreign-data-wrappers/socrata) directly through Splitgraph.
