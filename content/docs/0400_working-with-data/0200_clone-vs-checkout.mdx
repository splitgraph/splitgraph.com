export const meta = {
  id: "clone_vs_checkout",
  title: "Clone vs checkout"
};

A Splitgraph repository can be cloned using [`sgr clone`](../sgr/sharing-images/clone):
...
checkout requires it. This behaviour can be overridden by passing `--download-all` to `sgr clone`.
