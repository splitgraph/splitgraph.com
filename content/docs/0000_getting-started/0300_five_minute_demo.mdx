...
You need to be registered on [Splitgraph Cloud](../splitgraph_cloud/introduction) to pull and publish data. If you followed the [installation instructions](../installation), this should be already done for you. You can also use [`sgr cloud register`](../sgr/splitgraph_cloud/register) or [`sgr cloud login`](../sgr/splitgraph_cloud/login) to register or log in from the `sgr` client.

If you do not wish to register on Splitgraph Cloud, you can still use Splitgraph in a decentralized fashion, sharing data with other Splitgraph instances. See the [decentralized demo](./decentralized_demo) for an introduction that showcases importing data from CSV files and tracking changes to images, using Splitgraph's version control functionality.
...
Let's inspect this image remotely without cloning it, using [`sgr show`](../sgr/image_information/show).
The [`sgr` client](../concepts/sgr_client)
...
Let's also inspect the `precinct_results` [table](../sgr/image_information/table) in this image.
...
Table splitgraph/2016_election:3835145ada3f07cad99087d1b1071122d58c48783cbfe4694c101d35651fba90/precinct_resul
ts
...
Now let's actually [clone](../sgr/sharing_images/clone) the image. By default, this will only download the image's metadata.
...
## Querying the image
To interact with this image, you could [check it out](../working_with_data/clone_vs_checkout),
...
Let's check out the image into a so called ["layered querying"](../large_datasets/layered_querying) schema. This sets up a Postgres foreign data wrapper for this image that acts as a shim and presents itself as normal Postgres table to clients.
...
[`sgr sql`](../sgr/image_information/sql) is a shorthand to run an SQL query against the current engine. However, [any PostgreSQL client](../integrating_splitgraph/other_clients) will work with Splitgraph, allowing you to work with Splitgraph using your favourite tools, including [DataGrip](../integrating_splitgraph/datagrip), [pgAdmin](../integrating_splitgraph/pgadmin) or [other clients like pgcli or DBeaver](../integrating_splitgraph/other_clients).
...
[Build](../sgr/splitfile_execution/build) this image. Even though we have it pulled locally, this isn't necessary to build it: Splitgraph downloads required image fragments automatically.
...
Processing table sg_tmp_83918185ae99d2cf49cb10fbe42dd723
 ---> ee8dc4bb7c47
...
Splitfiles also support joins between Splitgraph images. For an example or to learn more about Splitfiles, check out the [Splitfile reference](../splitfiles/splitfile_intro) or some [sample Splitfiles](../working_with_data/using_splitfiles#sample-splitfiles).
...
Created at 2020-05-22T15:18:46.339966
Size: 6.38 KiB
...
```shell-session
$ sgr cloud metadata splitgraph-demo/votes_by_state dataset-metadata.yml
...
You also have an OpenAPI-compatible [REST API](/splitgraph-demo/votes_by_state/latest/-/api-schema) generated for the dataset, provided by [PostgREST](http://postgrest.org/en/latest/).



## Further reading

  * [Decentralized demo](/decentralized_demo)
  * [Splitfile reference](../splitfiles/splitfile_intro)
  * [Sample Splitfiles](https://github.com/splitgraph/splitgraph/tree/master/examples/sample_splitfiles)
  * [Splitgraph Cloud reference](../splitgraph_cloud/introduction)
