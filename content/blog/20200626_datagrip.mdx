...
How did we do this and is there anything other tools can learn from it?
...
## Enhancing abstractions without breaking them

For Splitgraph, we chose PostgreSQL as a foundation to build on. A Splitgraph engine is a PostgreSQL instance with some extra custom extensions. Most of Splitgraph's operations use the SQL standard. After a Splitgraph table is [checked out](/docs/sgr/image-management-creation/checkout), it's just an ordinary table that you can use all features of PostgreSQL on.
