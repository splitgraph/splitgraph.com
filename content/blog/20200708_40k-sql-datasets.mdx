export const meta = {
  ...
  related: ["foreign-data-wrappers", "introduction-to-splitgraph"],
};
...
The Splitgraph catalog classifies these datasets as [external repositories](/docs/splitgraph-cloud/external-repositories). These are different from the default [Splitgraph repositories](/docs/concepts/repositories), which are collections of [Splitgraph images](/docs/concepts/images). Yet Splitgraph allows you to query them in the same way as you do Splitgraph images. For example, you can use SQL to query any repository or [`JOIN` between them](/docs/ingesting-data/socrata#using-metabase-to-join-and-plot-data-from-multiple-data-portals). And every external repository includes an [auto-generated Postgrest API](/docs/splitgraph-cloud/publish-rest-api).
...
At this point, all the datasets in these two data portals are available for querying. You can query them in isolation, or you can query them together. For example, you can compare daily COVID cases in Chicago and Cambridge with a standard `JOIN` query:
...
With Splitgraph, there are two primary ways to ingest data: cloning it or mounting it. ["Cloning" (and checking-out)](/docs/working-with-data/clone-vs-checkout) an image means downloading a versioned data image, which is a snapshot of a database comprised of delta-compressed diffs. For example, the result of running a Splitfile is an image. On the other hand, "mounting" means establishing a connection to a live data source. The term comes from the idea of "mounting" a filesystem. A mounted table uses a [foreign data wrapper](/docs/ingesting-data/foreign-data-wrappers/introduction) (FDW), and you don't ingest data from it until you query it. For example, the [Socrata FDW](/docs/ingesting-data/socrata) translates SQL queries to [SoQL queries](https://dev.socrata.com/docs/queries/) and forwards them to the Socrata server.

Mounting is the key abstraction that allows Splitgraph Cloud to index external repositories with features like an auto-generated REST API. On the backend, the "materialization cluster" (a subject for a later post) uses the Splitgraph library and Socrata mount handler to mount repositories on demand. Then it exposes the mounted schemata to a customized version of [Postgrest](http://postgrest.org/) which creates the API. Separately, a periodic Airflow task queries the Socrata metadata API to discover and index over 40,000 repositories. Conveniently, the same Socrata software powers over 200 government open-data portals, so one mount handler provides a large catalog of useful live data.

## Avoiding the pull of Data Gravity

Mounting is a powerful abstraction because it allows you to interact directly with upstream data sources, avoiding the need for ETL. In 2010, GE Engineer Dave McCrory coined the term "[data gravity](https://datagravitas.com/2010/12/07/data-gravity-in-the-clouds/)." In his blog post, he observed that "data if large enough can be virtually impossible to move." Splitgraph, as a data versioning solution, should work with all your data, not just the subsets of it that you can move. This is why mounting data is core capability of Splitgraph. Traditional ETL tools force you to ingest or duplicate your data before you can interact with it. With Splitgraph, you only need to pull upstream data into your images at query time. This abstraction allows incremental adoption and quick experimentation; there is no need to move your data warehouse to start using Splitgraph. Instead, you only need to setup an FDW.
