...
The [decentralized demo](docs/getting-started/decentralized_demo) shows the basics of running Git operations on data.

Our [Jupyter/scikit-learn](docs/integrating_splitgraph/jupyter_notebooks) demo shows using Splitgraph
to switch between training and validation versions of a machine learning dataset, whereas the [dbt](docs/integrating_splitgraph/dbt)
...
Forget BI connectors or ETL jobs. With PostgreSQL [foreign data wrappers](docs/ingesting_data/foreign_data_wrappers/introduction),
...
[logical replication client](docs/ingesting_data/postgres_replication) and ingest data directly
...
dataset's history without sacrificing storage. See the [decentralized demo](docs/getting-started/decentralized_demo)
for an introduction to using Splitgraph in a peer-to-peer fashion.

[Splitgraph Cloud](docs/splitgraph_cloud/introduction) is the analog of the Docker registry for
Docker: it's a repository of publicly available data images that you can experiment with.

See the [five-minute demo](docs/getting-started/five_minute_demo) that will take you through
...
See the [five-minute demo](docs/getting-started/five_minute_demo) that shows you how to build a
...
Splitfile that joins several source data images in multiple stages or the [PostGIS example](docs/integrating_splitgraph/postgis)
...
feature parity with PostgreSQL. [Layered querying](docs/large_datasets/layered_querying) expands
...
Check our our [five-minute demo](docs/getting-started/five_minute_demo) for an introduction on
using Splitgraph to query public data. The [Metabase example](docs/integrating_splitgraph/metabase)
